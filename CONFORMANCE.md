...
| cvc-identity-constraint | §3.11.4 | V3 | wip (unique/key/keyref over the selector/field subset; local-name name tests; value-space field comparison; keyref resolved against per-instance subtree-scoped node tables flushed post-walk — a keyref on E sees keys sourced within E's subtree only, upward propagation §3.11.5) | xsdvalidate/idc.go |
