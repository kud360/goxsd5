...
| derivation-ok-restriction | §3.4.6 | M6/M9 | wip (base {final} blocks restriction; attribute-use/inheritability restriction; attribute-wildcard subset; open-content subset clause 9; clause 2.4.2 particle subsumption (delegating to §3.4.6.4 Content type restricts) is enforced under cos-particle-restrict via `checkParticleRestrict`, wired from `runStaticTypeChecks` at parser/buildschema.go:219) | parser/restrict.go |
...
| cvc-identity-constraint | §3.11.4 | V3 | wip (unique/key/keyref over the selector/field subset; name tests resolved per XPath 2.0 / §3.11.6 — a prefixed step compares namespace URI + local via the constraint's in-scope namespace bindings; an unprefixed element step resolves against the default element namespace from xpathDefaultNamespace (§3.13.6.2 clause 2.2.3, §3.13.2), falling back to local-name only when none is in effect; unprefixed attribute steps stay local-name only (the default namespace applies to elements, not attributes); value-space field comparison; keyref resolved against per-instance subtree-scoped node tables flushed post-walk — a keyref on E sees keys sourced within E's subtree only, upward propagation §3.11.5) | xsdvalidate/idc.go |
| cvc-assertion | §3.13.4 | V4 | wip (xs:assert + xs:alternative/CTA over a partial XPath subset, fail-open; function library covers abs/floor/ceiling/round (fn:round half-to-+∞), substring/substring-before/substring-after (1-based, fn:round arg semantics) and matches (unanchored, via xsdregex, `i` flag only; `s`/`m`/`x`/`q`/any other flag ⇒ dynamic error ⇒ assertion unsatisfied) alongside the existing string/numeric/date/node accessors, with type-mismatch / bad-pattern raising dynamic errors ⇒ unsatisfied not fail-open; inheritable attributes; simple-type xs:assertion with schema-typed $value — list items recurse into ItemType, union requires a member to satisfy its own assertions, attribute values included; complex simpleContent xs:assert binds $value to the content sequence; no context item for simple-type assertions so `.`/position()/last() and failed constructor casts raise dynamic errors ⇒ unsatisfied; simple-type recursion in assess.go assertSatisfied; SUBSET BOUNDARY — assertion `fn:matches` reuses the XSD Part 2 Appendix-G regex grammar (via xsdregex), NOT the F&O/XQuery 2.0 regex grammar that XSD 1.1 normatively binds it to, so `^`/`$` are LITERAL characters with no anchoring, there are no back-references or reluctant quantifiers, and only the `i` flag is honoured — `s`/`m`/`x`/`q` and any other flag raise a dynamic error ⇒ assertion unsatisfied; full F&O-flavor `fn:matches` is a documented future gap) | xsdvalidate/assert.go |
...
| src-expredef | §4.2.4 | M9+ | deferred (redefine/override is a legal redefinition) | |
